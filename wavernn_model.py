...
def forward_model(model, coarse_samples, fine_samples, hidden):
    with torch.no_grad():
        x_coarse = torch.FloatTensor(coarse_samples[:,:-1])
        # print(x_coarse.shape)
        x_coarse = x_coarse / 127.5 - 1.
        x_fine = torch.FloatTensor(fine_samples[:,:-1])
        x_fine = x_fine / 127.5 - 1.
        
        y_coarse = torch.LongTensor(coarse_samples[:,1:])
        y_fine = torch.LongTensor(fine_samples[:,1:])
        seq_len = coarse_samples.shape[1]
        #print('coarse')
        ...
        for i in range(seq_len-1):
            ...
            #print('aaa')
            #print(hidden)
        #TODO walkthrough for the last step
        return hidden
...
def train(model, coarse_classes, fine_classes, optimizer, num_steps, batch_size, lr=1e-3, seq_len=960, target_sparsity=.95, t_0=0, num_steps_sparsity_update=1) :
    ...
    for step in range(num_steps) :
        ...
        loss = 0
        hidden = model.init_hidden(batch_size, cuda=True)
        optimizer.zero_grad()
        #print(coarse_classes.shape)
        ...
        if False and step >= t_0 and step % num_steps_sparsity_update == 0:
...
def generate2(model, seq_len, hidden=None) :
...
def generate(model, seq_len, hidden=None, init_coarse=0, init_fine=0) :
    ...
    with torch.no_grad() :
        ...
        #
        # Some initial inputs
        out_coarse = torch.LongTensor(init_coarse)#.cuda()
        out_fine = torch.LongTensor(init_fine)#.cuda()
        #
        # if hidden is None:
        #     # We'll need a hidden state
        #     hidden = model.init_hidden()
